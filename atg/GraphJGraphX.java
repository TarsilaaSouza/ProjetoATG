...
public class GraphJGraphX extends JApplet {
    ...
    public void init() {
        ...
        ListenableGraph<String, MyEdge> g =
        	new DefaultListenableGraph<> (new SimpleWeightedGraph<>(MyEdge.class));
        ...
        jgxAdapter.getStylesheet().getDefaultEdgeStyle()
        	.put(mxConstants.STYLE_ENDARROW, "none");
	...
	private void setAllVertexAndEdges (ListenableGraph<String, MyEdge> g) {
		...
		for (Aresta a: grafo.getArestas()) {
			g.addEdge(a.getV1().toString(), 
					a.getV2().toString());
			
			Double p = Double.valueOf(new Float(a.getPeso()).toString());
			g.setEdgeWeight(g.getEdge(a.getV1().toString(), 
					a.getV2().toString()), p);
		}
	}   
}
